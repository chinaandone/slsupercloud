...
public class ImageServlet extends HttpServlet {
	 ...
	 *
	 */
	private static final long serialVersionUID = 8457150133043424793L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String s = "abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789"; // 设置备选验证码:包括"a-z"和数字"0-9"
		String sRand = "";
		...
		for (int i = 0; i < length; i++) {
			...
			// drawString(a,x,y)a为要画出来的东西，x和y表示要画的东西最左侧字符的基线位于此图形上下文坐标系的 (x, y)
			// 位置处
		}
		// 将生成的字符串存储在session中
		...
		if("login".equals(type)){
			...
			session.setAttribute(Constants.MEMBER_LOGIN_SECURITY_CODE, sRand);
		}else if("register".equals(type)){
			session.setAttribute(Constants.MEMBER_REGISTER_SECURITY_CODE, sRand);
		}else if("forgetPassword".equals(type)){
			session.setAttribute(Constants.MEMBER_FORGETPASSWORD_SECURITY_CODE, sRand);
		}else if("register4site".equals(type)){
			session.setAttribute(Constants.MEMBER_REGISTER_SECURITY_CODE_4SITE, sRand);
		}else if("login4site".equals(type)){
			session.setAttribute(Constants.MEMBER_LOGIN_SECURITY_CODE_4SITE, sRand);
		}
		g.dispose();// 图像生效
		// 禁止图像缓存
