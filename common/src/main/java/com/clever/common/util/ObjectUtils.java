...
public class ObjectUtils {
    ...
    /**
     * 验证且设置商品金额
     * 目前商品金额的策略 是可以为空(面议） 如果不为空 则验证范围  >0 <=9999999.99
     *
     * @param priceStr
     * @return
     * @throws DataValidateException
     */
    public static Long checkAndSetProductPrice(String priceStr) throws DataValidateException {

        if (StringUtils.isNotBlank(priceStr)) {
            boolean valid = ObjectUtils.validPrice(priceStr);
            if (!valid) {
                throw new DataValidateException("请输入大于0小于9999999.99的单价金额，金额最多2位小数。");
            }
            Double dValue = Double.parseDouble(priceStr);
            Long value = BigDecimalUtil.mul(dValue, 100, 0).longValue();

            if (value <= 0l) {
                throw new DataValidateException("请输入大于0小于9999999.99的单价金额");
            }
            return value;
    ...
    public static boolean validPrice(String str) {
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("^(([1-9]{1}\\d{0,6})|([0]{1}))(\\.(\\d){0,2})?$"); // 判断小数点后一位的数字的正则表达式
    ...
    /**
     * 验证且设置商品名称
     * 目前验证策略 ：不能为空 1-30位 中英文数字
     *
     * @param productName
     * @return
     * @throws DataValidateException
     */
    public static String checkAndSetProductName(String productName) throws DataValidateException {

        if (StringUtils.isBlank(productName)) {
            throw new DataValidateException("商品名称不能为空");
        }
        if (StringUtils.length(productName) > 30) {
            logger.error("商品名称[" + productName + "]长度超过30个字符");
            throw new DataValidateException("商品名称超过30个字");
        }
        // 半角/全角 字母 数字  及汉字  http://unicode-table.com/en/#FF5A
        boolean b = Pattern.matches(PRODUCT_NAME_PATTERN, productName);
        if (!b) {
            logger.error("商品名称[" + productName + "]含有特殊字符");
            throw new DataValidateException("商品名称只能为30字以内中、英文或数字");
        }
        /*String newcharacter = StringUtils.removePattern(productName, "[^(a-zA-Z0-9\\u4e00-\\u9fa5\\uff10-\\uff19\\uff21-\\uff3a\\uff41-\\uff5a)]");
        if (StringUtils.length(productName) != StringUtils.length(newcharacter)) {
            logger.error("标签名称[" + productName + "]含有特殊字符");
            throw new DataValidateException("商品名称格式不正确");
        }*/
        return productName;
    }
